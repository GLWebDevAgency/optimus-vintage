...
export function validateEnv(): void {
  const missing = requiredEnvVars.filter(key => !process.env[key] && !process.env.DATABASE_URL);
  if (missing.length > 0 && !process.env.DATABASE_URL) {
    console.warn(`⚠️ Missing environment variables: ${missing.join(', ')}`);
    console.warn('Using default development configuration. DO NOT use in production!');
  }
}
