...
| Données | Migrations versionnées, additives, appliquées explicitement en production | ✅ 4 migrations (`0000_init` → `0003_billing_state_idempotency`), `pnpm db:migrate` réel ou `CHINE_AUTO_MIGRATE` explicite | `packages/infrastructure/drizzle/*.sql`, `scripts/migrate.ts` |
