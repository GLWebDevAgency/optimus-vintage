...
export interface PeriodReport {
  ...
  readonly byPlatform: ReadonlyArray<{ platform: Platform; count: number; net: Money; margin: Money }>;
...
export function computePeriodReport(sales: readonly Sale[], from: IsoDate, to: IsoDate, currency: Currency): PeriodReport {
  ...
  const gross = sum((s) => s.economics.gross), fees = sum((s) => s.economics.fees), costs = sum((s) => s.economics.costs);
  const net = sum((s) => s.economics.net), acquisition = sum((s) => s.acquisitionCost), margin = net.subtract(acquisition);
  ...
  for (const s of valid) {
    ...
    byPlatformMap.set(s.platform, { count: p.count + 1, net: p.net.add(e.net), margin: p.margin.add(e.margin) });
    const d = byDayMap.get(s.soldAt) ?? { net: zero, margin: zero, count: 0 };
    byDayMap.set(s.soldAt, { net: d.net.add(e.net), margin: d.margin.add(e.margin), count: d.count + 1 });
  }
  return {
    from, to, salesCount: valid.length, gross, fees, costs, net, acquisition, margin,
    ...
    byPlatform: [...byPlatformMap.entries()].map(([platform, v]) => ({ platform, ...v })).sort((a, b) => b.net.minor - a.net.minor),
    byDay: [...byDayMap.entries()].map(([day, v]) => ({ day, ...v })).sort((a, b) => (a.day < b.day ? -1 : 1)),
  };
}
