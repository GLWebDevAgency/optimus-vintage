...
export const PLATFORMS = ["VINTED", "VESTIAIRE", "LEBONCOIN", "DEPOP", "EBAY", "ETSY", "WHATNOT", "INSTAGRAM", "IN_PERSON", "OTHER"] as const;
export type Platform = (typeof PLATFORMS)[number];

export const PLATFORM_LABEL: Readonly<Record<Platform, string>> = {
  VINTED: "Vinted", VESTIAIRE: "Vestiaire Collective", LEBONCOIN: "Leboncoin", DEPOP: "Depop", EBAY: "eBay",
  ETSY: "Etsy", WHATNOT: "Whatnot", INSTAGRAM: "Instagram", IN_PERSON: "En main propre", OTHER: "Autre",
};
...
export const DEFAULT_FEE_SCHEDULES: Readonly<Record<Platform, FeeSchedule>> = {
  VINTED: { percent: 0, fixedMinor: 0, note: "Aucun frais vendeur ; la protection acheteur est payée par l'acheteur." },
  VESTIAIRE: { percent: 15, fixedMinor: 0, minMinor: 1500, note: "Commission vendeur 15 %, minimum 15 €." },
  LEBONCOIN: { percent: 0, fixedMinor: 0, note: "Gratuit hors options de mise en avant." },
  DEPOP: { percent: 0, fixedMinor: 0, note: "Plus de frais vendeur depuis 2024 (frais reportés sur l'acheteur)." },
...
export class ScheduleFeePolicy implements PlatformFeePolicy {
  constructor(private readonly schedules: Readonly<Record<Platform, FeeSchedule>> = DEFAULT_FEE_SCHEDULES) {}
  ...
  scheduleFor(platform: Platform): FeeSchedule { return this.schedules[platform]; }
