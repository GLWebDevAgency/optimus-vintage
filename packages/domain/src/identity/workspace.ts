...
export class Workspace {
  ...
  static create(p: WorkspaceProps): Workspace {
    ...
    assertInvariant(
      /^[A-Z]{1,4}$/.test(p.skuPrefix),
      "Le préfixe SKU doit faire 1 à 4 lettres majuscules",
      { skuPrefix: p.skuPrefix },
    );
    if (p.targetMargin.kind === "PERCENT")
      assertInvariant(
        p.targetMargin.value >= 0 && p.targetMargin.value <= 1000,
        "Marge cible % hors bornes",
      );
    return new Workspace(p);
  }
  static rehydrate(p: WorkspaceProps): Workspace {
    return new Workspace(p);
  }
  ...
  get currency(): Currency {
    return this.p.currency;
  }
  get locale(): "fr" | "en" | "de" {
    return this.p.locale;
  }
  get targetMargin(): TargetMargin {
    return this.p.targetMargin;
  }
  get plan(): Plan {
    return this.p.plan;
  }
  get skuPrefix(): string {
    return this.p.skuPrefix;
  }
  get createdAt(): Date {
  ...
  with(
    patch: Partial<
      Pick<WorkspaceProps, "name" | "currency" | "locale" | "targetMargin" | "plan" | "skuPrefix">
    >,
  ): Workspace {
