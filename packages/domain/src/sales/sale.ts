...
export class Sale extends AggregateRoot<SaleId> {
  private constructor(private p: SaleProps) {
    super(p.id);
  }

  static record(input: RecordSaleInput, feePolicy: PlatformFeePolicy): Result<Sale> {
    const c = input.grossPrice.currency;
    if (input.grossPrice.isNegative)
      return err(new DomainError("NEGATIVE_PRICE", "Le prix de vente ne peut pas être négatif"));
    if (input.acquisitionCost.currency !== c)
      return err(new DomainError("CURRENCY_MISMATCH", "Coût d'acquisition dans une autre devise"));
    const zero = Money.zero(c);
    const shipping = input.shippingCost ?? zero,
      packaging = input.packagingCost ?? zero,
      other = input.otherCosts ?? zero;
    for (const m of [shipping, packaging, other]) {
      if (m.currency !== c)
        return err(new DomainError("CURRENCY_MISMATCH", "Frais dans une autre devise"));
      if (m.isNegative)
        return err(new DomainError("NEGATIVE_COST", "Un frais ne peut pas être négatif"));
    }
    const fees = input.platformFeesOverride ?? feePolicy.feesFor(input.platform, input.grossPrice);
    if (fees.currency !== c || fees.isNegative)
      return err(new DomainError("INVALID_FEES", "Frais plateforme invalides"));
    ...
    if (status === "COMPLETED")
      sale.record({
        ...
        itemId: input.itemId,
        saleId: input.id,
        net: sale.economics.net,
        occurredAt: input.now,
      });
    return ok(sale);
  }
  static rehydrate(p: SaleProps): Sale {
    return new Sale(p);
  }
  ...
  get grossPrice(): Money {
    return this.p.grossPrice;
  }
  get platformFees(): Money {
    return this.p.platformFees;
  }
  get shippingCost(): Money {
    return this.p.shippingCost;
  }
  get packagingCost(): Money {
    return this.p.packagingCost;
  }
  get otherCosts(): Money {
    return this.p.otherCosts;
  }
  ...
  get countsAsRevenue(): boolean {
    return this.p.status === "COMPLETED" || this.p.status === "PENDING";
  }
  ...
  get economics(): SaleEconomics {
    ...
    return {
      ...
      net,
      margin,
      roi: margin.ratioTo(this.p.acquisitionCost),
  ...
  complete(now: Date): Result<void> {
    if (this.p.status !== "PENDING")
      return err(new InvalidTransition("Sale", this.p.status, "COMPLETED"));
  ...
  cancel(now: Date): Result<void> {
    if (this.p.status === "CANCELLED" || this.p.status === "REFUNDED")
      return err(new InvalidTransition("Sale", this.p.status, "CANCELLED"));
  ...
  refund(now: Date): Result<void> {
    if (this.p.status !== "COMPLETED")
      return err(new InvalidTransition("Sale", this.p.status, "REFUNDED"));
  ...
  update(
    ...
    for (const m of [
      ...
      if (m.currency !== this.p.grossPrice.currency)
        return err(new DomainError("CURRENCY_MISMATCH", "Devise incohérente"));
