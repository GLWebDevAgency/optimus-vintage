...
const item = (id: string, cost: number, sourceId = "pal") =>
  unwrap(
    Item.create({
      id: asItemId(id),
      workspaceId: ws,
      sourceId: asSourceId(sourceId),
      sku: `CH-${id.padStart(4, "0")}`,
      title: `Pièce ${id}`,
      category: "OTHER",
      condition: "GOOD",
      acquisitionCost: eur(cost),
      now,
    }),
  );
    ...
    Sale.record(
      {
        ...
        sourceId: asSourceId("pal"),
        platform,
        grossPrice: eur(gross),
        acquisitionCost: eur(5),
        soldAt,
...
describe("Performance d'une source et prix plancher", () => {
  it("palette 240 €, 48 pièces : après 31 ventes à 8,66 net, il reste à récupérer et un floor price", () => {
    ...
    const sales = Array.from({ length: 31 }, (_, i) =>
      sale(`s${i}`, String(i + 1), 8.66, "2026-09-01"),
    );
    for (let i = 0; i < 31; i++)
      unwrap(items[i]?.markSold(now) ?? { ok: false, error: new Error("no") });
...
describe("Rapport de période", () => {
  it("agrège par plateforme et par jour, ignore les remboursées", () => {
    const sales = [
      sale("a", "1", 75, "2026-09-01"),
      sale("b", "2", 40, "2026-09-02", "EBAY"),
      sale("c", "3", 30, "2026-08-30"),
    ];
