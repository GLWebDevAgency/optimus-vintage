...
export function Segmented<V extends string>({ options, value, onChange, className, size = "md", ...rest }: SegmentedProps<V>) {
  const id = useId();
  ...
  return (
    <LayoutGroup id={id}>
      <div role="radiogroup" aria-label={rest["aria-label"]} onKeyDown={onKeyDown} className={cn("flex rounded-field border border-line bg-surface p-[3px]", className)}>
        {options.map((o) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                size === "md" ? "min-h-[38px] px-2 text-[12.5px]" : "min-h-[32px] px-2 text-[11.5px]",
