...
export function ListRow({
  ...
  const autoTone: Tone =
    tone ??
    (isMoney ? (amount.minor > 0 ? "pos" : amount.minor < 0 ? "neg" : "neutral") : "neutral");
    ...
    <>
      <span className="grid h-[38px] w-[38px] shrink-0 place-items-center overflow-hidden rounded-[9px] bg-surface-2 text-ink-3">
        {typeof thumb === "string" ? (
          ...
          (thumb ?? <AppIcon name="shirt" size={18} />)
        )}
      </span>
      <span className="min-w-0">
        <span className="block truncate font-ui text-[14px] font-semibold leading-tight text-ink">
          {title}
        </span>
        {sub ? (
          <span className="mt-0.5 block truncate font-mono text-[11.5px] text-ink-3">{sub}</span>
        ) : null}
      </span>
      <span className="flex items-center gap-2">
        {amountText !== undefined ? (
          <span className={cn("font-mono text-[14px] font-medium tabular", toneClass[autoTone])}>
            {amountText}
          </span>
        ) : null}
        {trailing}
  ...
  const classes = cn(
    ...
    interactive &&
      "transition-colors duration-micro hover:bg-surface-2 active:bg-surface-2 focus-thread",
    className,
  );
  if (href) {
    const L = Link ?? "a";
...
export function List({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "grid divide-y divide-line overflow-hidden rounded-card border border-line bg-surface",
        className,
      )}
    >
      {children}
