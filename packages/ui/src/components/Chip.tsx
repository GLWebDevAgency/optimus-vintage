...
export function Chip({ selected = false, leading, size = "md", className, children, type = "button", ...rest }: ChipProps) {
  return (
    <button
      type={type}
      ...
      className={cn(
        ...
        selected ? "bg-ink text-bg border-ink" : "bg-transparent text-ink border-line-2 hover:border-ink",
        className,
      )}
      {...rest}
    >
...
export type ChipGroupProps<V extends string> = {
  ...
  | { readonly mode?: "single"; readonly value: V | null; readonly onChange: (value: V | null) => void; readonly allowEmpty?: boolean }
  | { readonly mode: "multi"; readonly value: readonly V[]; readonly onChange: (value: V[]) => void }
);
...
export function ChipGroup<V extends string>(props: ChipGroupProps<V>) {
  ...
  return (
    <div
      ...
      className={cn("flex gap-2", scroll ? "overflow-x-auto no-scrollbar -mx-5 px-5 py-1" : "flex-wrap", className)}
    >
      {options.map((o) => (
        <Chip key={o.value} selected={isSelected(o.value)} disabled={o.disabled} leading={o.leading} size={size} onClick={() => toggle(o.value)}>
          {o.label}
        </Chip>
      ))}
    </div>
